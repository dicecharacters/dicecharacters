...
//
// Shared LF-Masken/Compile in dieser Datei werden vom Subclass Builder
// mitgenutzt; Extraktion nach shared.js bei Bedarf schrittweise.
//=======================================================================

//=======================================================================
// CUSTOM_CLASS_CONFIG – zentrale Schnell-Einstellungen (Identity / Limits)
//=======================================================================
// Hier Limits und IDs anpassen (z. B. idMin, equipMaxItems).
// Weitere Feature-Configs weiter unten in DIESER Datei (bewusst getrennt):
//   CUSTOM_CLASS_LF_CONFIG  – Stufenmerkmale Tab 2 (Zeilen/Presets/Limits)
//   CUSTOM_CLASS_SC_CONFIG  – Unterklassen-Editor Tab 3
// Flache Aliase darunter halten bestehenden Code stabil (nur Umschichtung).
//=======================================================================
const CUSTOM_CLASS_CONFIG = Object.freeze({
    /**
     * Mindest-ID für Custom Classes.
     * PHB-Klassen liegen darunter; nextCustomClassId() startet hier.
     */
    idMin: 100,

    /** Max. Zeichen Klassenname (pro Sprache) */
    nameMax: 30,
    /** Max. Zeichen Klassenbeschreibung (pro Sprache) */
    descMax: 500,

    /** Max. Gegenstände pro Startausrüstungs-Option (A/B/C) */
    equipMaxItems: 10,
    /** Max. Stückzahl eines Gegenstands in der Equip-Zeile */
    amountMax: 999,
    /** Max. GP-Feld pro Equip-Option */
    goldMax: 999,
    /**
     * Startausrüstungs-Optionen im Class Builder.
     * Background-Builder: nur A/B (ohne C) – dort eigene Config.
     */
    equipOptionKeys: Object.freeze(["A", "B", "C"]),

    /** Zauberfokus-Optionen (inkl. Zauberbuch) */
    focusOptions: Object.freeze([
        "arcaneFocusLabel",
        "druidicFocusLabel",
        "holySymbolLabel",
    ...
    /** Oberflächen-Werkzeuge: konkrete Auswahl erst in Schritt 6 */
    toolSurfaceOptions: Object.freeze([
        "artisansToolsLabel",
        "musicalInstrumentLabel",
        "gamingSetLabel"
    ]),

    /** Erlaubte Zauberattribute (INT / WEI / CHA) */
    spellcastingAbilityLabels: Object.freeze([
        "intelligenceLabel",
        "wisdomLabel",
        "charismaLabel"
    ])
...
const CUSTOM_CLASS_ID_MIN = CUSTOM_CLASS_CONFIG.idMin;
const CUSTOM_CLASS_FOCUS_OPTIONS = CUSTOM_CLASS_CONFIG.focusOptions;
const CUSTOM_CLASS_TOOL_SURFACE_OPTIONS = CUSTOM_CLASS_CONFIG.toolSurfaceOptions;
const CUSTOM_CLASS_NAME_MAX = CUSTOM_CLASS_CONFIG.nameMax;
const CUSTOM_CLASS_DESC_MAX = CUSTOM_CLASS_CONFIG.descMax;
...
function getLfSpellcastingAbilityOptions() {
    const allowed = new Set(LF_SPELLCASTING_ABILITY_LABELS);
    const all = (typeof attributeList !== "undefined" && Array.isArray(attributeList))
        ? attributeList
        : [];
    return all.filter(a => allowed.has(a.translationLabel));
}

/**
 * Aktives Merkmals-Float (Class Builder oder CSC) – vermeidet ID-Kollisionen
 * zwischen #ccLfFloatOverlay und #cscLfFloatOverlay.
 */
function getActiveLfFloatRootEl() {
    if (typeof getActiveLfFloatOverlayEl === "function") {
        const overlay = getActiveLfFloatOverlayEl();
        if (overlay) return overlay;
    }
    return document.getElementById("ccLfFloatOverlay")
        || document.getElementById("cscLfFloatOverlay")
        || document;
}

function getActiveLfFloatBodyEl() {
    const root = getActiveLfFloatRootEl();
    if (root && root.querySelector) {
        const body = root.querySelector(".cc-lf-float-body");
        if (body) return body;
    }
    return document.getElementById("ccLfFloatBody")
        || document.getElementById("cscLfFloatBody");
}

function queryActiveLfFloat(selector) {
    const root = getActiveLfFloatRootEl();
    if (root && root.querySelector) return root.querySelector(selector);
    return document.querySelector(selector);
}

function queryAllActiveLfFloat(selector) {
    const root = getActiveLfFloatRootEl();
    if (root && root.querySelectorAll) return root.querySelectorAll(selector);
...
/**
 * Zauberbuch-Mechanik aktiv?
 * - Class Builder: Fokus enthält spellbookLabel
 * - CSC / Compile mit targetClassSlug: PHB-Magier oder Custom-Eltern mit Zauberbuch-Fokus
 */
function customClassStateUsesSpellbook(state) {
    const st = state
        || ((typeof isCustomSubclassEditorActive === "function" && isCustomSubclassEditorActive()
            && typeof customSubclassEditorState !== "undefined")
            ? customSubclassEditorState
            : (typeof customClassEditorState !== "undefined" ? customClassEditorState : null));

    if (normalizeToArray(st?.spellcastingFocus).includes("spellbookLabel")) return true;

    const parentSlug = st?.targetClassSlug
        || ((typeof isCustomSubclassEditorActive === "function" && isCustomSubclassEditorActive()
            && typeof getCscParentClassSlug === "function")
            ? getCscParentClassSlug()
            : "");
    if (!parentSlug) return false;
    if (String(parentSlug).toLowerCase() === "wizard") return true;
    const core = (typeof classCoreTraitsList !== "undefined" && Array.isArray(classCoreTraitsList))
        ? classCoreTraitsList.find(c =>
            String(c.translationLabel || "").toLowerCase() === String(parentSlug).toLowerCase()
        )
        : null;
    return normalizeToArray(core?.spellcastingFocus).includes("spellbookLabel");
}
...
/**
 * Editor-State für Sprachverfügbarkeit der Merkmalsmasken
 * (Custom Class Builder oder Standalone-Unterklassen-Builder).
 */
function getActiveLfEditorLangState() {
    if (typeof isCustomSubclassEditorActive === "function" && isCustomSubclassEditorActive()
        && typeof customSubclassEditorState !== "undefined" && customSubclassEditorState) {
        return customSubclassEditorState;
    }
    return customClassEditorState;
}

function isCustomClassLangAvailable(lang) {
    return ensureAvailableLanguages(getActiveLfEditorLangState()).includes(lang);
}
...
function backToCustomClassChooser() {
    ...
    document.getElementById("customClassEditorView").style.display = "none";
    if (typeof setCustomFeatureModalChooserMode === "function") {
        setCustomFeatureModalChooserMode("customClassOverlay", true);
    }
}
...
function showCustomClassEditor() {
    ...
    if (typeof setCustomFeatureModalChooserMode === "function") {
        setCustomFeatureModalChooserMode("customClassOverlay", false);
    }
...
function renderCustomClassLangAvailabilityRow() {
    return renderLangAvailabilityRowHtml(customClassEditorState, {
        inputName: "ccLangAvail",
        onChange: "onCustomClassLangAvailabilityChange()"
    });
}

/**
 * Sprachen-Checkboxen (wie Tab 1) – state-agnostisch für Class- und Subclass-Builder.
 * @param {object} state – Editor-State mit availableLanguages
 * @param {{ inputName: string, onChange: string }} opts
 */
function renderLangAvailabilityRowHtml(state, opts) {
    const inputName = opts?.inputName || "ccLangAvail";
    const onChange = opts?.onChange || "onCustomClassLangAvailabilityChange()";
    const activeLang = getActiveUiLang();
    const available = ensureAvailableLanguages(state);
    const checks = getCustomClassSupportedLangs().map(lang => {
        ...
        return `<label class="cc-lang-avail-item${lockedClass}">
            <input type="checkbox" name="${inputName}" value="${lang}" ${checked} ${disabled}
                onchange="${onChange}">
    ...
    return `
        <div class="custom-class-field custom-class-lang-availability">
            <div class="custom-class-lang-avail-row">
                <span class="custom-class-section-title">${tCC("cfLangAvailabilityLabel")}</span>
...
function renderCustomClassTab1() {
    ...
    container.innerHTML = `
        ...
        <div class="custom-class-field">
            <label for="ccHitDie">${tCC("hitDiceLabel")} ${req}</label>
        ...
        <div class="custom-class-field">
            <div class="custom-class-section-title">${tCC("weaponCategoriesLabel")}</div>
            <div class="custom-class-check-grid" id="ccWeaponCatGrid"></div>
        </div>

        <div class="custom-class-field">
            <div class="custom-class-section-title">${tCC("weaponPropertiesLabel")}</div>
        ...
        <div class="custom-class-field">
            <label for="ccToolSelect">${tCC("toolsLabel")}</label>
    ...
    return `
        <div class="custom-class-lang-block" data-lang="${lang}">
            ...
            <div id="ccLangBody_${lang}" class="custom-class-lang-body ${collapsedClass}">
                <label for="ccName_${lang}">${tCC("cfNameLabel")} ${lang === activeLang ? '<span class="custom-class-required">*</span>' : ""}</label>
                ...
                <label for="ccDesc_${lang}" style="margin-top:8px;display:block;">${tCC("cfDescLabel")}</label>
...
function fillSpellcastingFields() {
    ...
    const abilityOptions = getLfSpellcastingAbilityOptions();
    // Ungültige Altwerte (z. B. Stärke) verwerfen
    if (customClassEditorState.spellcastingAbility
        && !abilityOptions.some(a => a.translationLabel === customClassEditorState.spellcastingAbility)) {
        customClassEditorState.spellcastingAbility = 0;
    }
    abilitySelect.innerHTML = `<option value="">${tCC("pleaseSelectLabel")}</option>`
        + abilityOptions.map(attr => {
    ...
    return `
        <div class="custom-class-equip-option" data-option="${optionKey}">
            ...
            <div id="ccEquipBody_${optionKey}">
                ...
                <button type="button" id="ccEquipAddBtn_${optionKey}" class="custom-class-add-item-btn" onclick="addEquipmentRow('${optionKey}')">${tCC("cfAddItemLabel")}</button>
                <div class="custom-class-equip-gp" style="margin-top:10px;">
                    <label for="ccEquipGP_${optionKey}" class="custom-class-equip-gp-label">${tCC("cfGoldGPLabel")}</label>
...
function slugifyClassName(name) {
    // Umlaute vor Diakritik-Strip: Ä→ae, Ö→oe, Ü→ue, ß→ss (sonst wird „Kämpfer“ zu „kampfer“)
...
function buildCoreTraitsFromState(state, slug, id) {
    return {
        ...
        isCustom: true,
        // Custom-Klassen: Quelle DiceCharacters (nicht PHB/SRD)
...
function isLfSlotEligibleForCompile(slot) {
    ...
    // Einfach → Vordefiniert (Custom Unarmored Defense): exakt 2 unterschiedliche Attribute nötig
    if (type === "simple" && cat === "preDefined"
        && slot.payload.optionsConfig?.preDefinedLabel === "customUnarmoredDefense") {
        const sel = Array.isArray(slot.payload.optionsConfig?.selectedAbilities)
            ? slot.payload.optionsConfig.selectedAbilities.filter(Boolean)
            : [];
        return sel.length === 2 && new Set(sel).size === 2;
    }
    ...
    if (type === "simple" && cat === "weaponTraining") {
        const cfg = slot.payload.optionsConfig || {};
        const core = getActiveLfCoreTraitsState();
        const catMode = cfg.weaponCategoryMode || "selection";
        const propMode = cfg.weaponPropertyMode || "selection";
        if (catMode === "all" && getLfRemainingWeaponCategoryNumbers(core).length) return true;
        if (propMode === "all" && getLfRemainingWeaponPropertyCategoryNumbers(core).length) return true;
    ...
    if (type === "simple" && cat === "armorTraining") {
        const cfg = slot.payload.optionsConfig || {};
        const core = getActiveLfCoreTraitsState();
        if (cfg.mode === "all" && getLfRemainingArmorCategoryNumbers(core).length) return true;
...
function applyCompiledCustomTexts(row, slot, keyStem, translationsBag, state, compileMeta) {
    ...
    // Einfach→Vordefiniert (Custom Unarmored Defense): Text mit gewählten Attributen instanzieren
    if (type === "simple" && cat === "preDefined" && cfg.preDefinedLabel === "customUnarmoredDefense") {
        const abilities = normalizeLfPreDefinedSelectedAbilities(
            cfg.selectedAbilities,
            getLfPreDefinedFeatureMeta("customUnarmoredDefense")
        );
        const shortKey = `${keyStem}ShortD`;
        const longKey = `${keyStem}D`;
        putCompiledTranslation(
            translationsBag, shortKey,
            formatLfCustomUnarmoredDefenseDesc(abilities, "de", "short"),
            formatLfCustomUnarmoredDefenseDesc(abilities, "en", "short")
        );
        putCompiledTranslation(
            translationsBag, longKey,
            formatLfCustomUnarmoredDefenseDesc(abilities, "de", "long"),
            formatLfCustomUnarmoredDefenseDesc(abilities, "en", "long")
        );
        row.classFeatureShortDescription = shortKey;
        row.classFeatureDescription = longKey;
        row.classFeaturesCharacterSheet = 3;
        row.unarmoredDefenseAbilities = abilities;
        // Basisklasse: wie Mönch/Barbar in Liste/Infobox; Unterklasse: bestehende UC-Regel (0/0)
        if (!(row.subclassCategoryNumber > 0)) {
...
/**
 * CSC: Zähler an bestehende Parent-ClassData anbinden (keine Kollision mit skill1…).
 */
function seedCompiledChoiceCountersFromParentClass(parentSlug) {
    const counters = createCompiledChoiceCounters();
    const data = (typeof getClassData === "function")
        ? (getClassData(String(parentSlug || "").toLowerCase(), "class") || [])
        : [];
    const bump = (key, n) => {
        if (Number.isFinite(n) && n > counters[key]) counters[key] = n;
    };
    const map = {
        skill: "skill",
        expertise: "expertise",
    ...
    (data || []).forEach(row => {
        // CSC-eigene Zeilen (CUSTOM_SUBCLASS_CATEGORY_NUMBER) nicht als Seed zählen
        const cscNum = (typeof CUSTOM_SUBCLASS_CATEGORY_NUMBER !== "undefined")
            ? CUSTOM_SUBCLASS_CATEGORY_NUMBER
            : 100;
        if ((row.subclassCategoryNumber || 0) === cscNum) return;
        const c = row.constForChoice;
        if (typeof c !== "string") return;
        c.split(";").forEach(part => {
            const m = String(part).trim().match(/^selected([A-Za-z]+)(\d+)$/i);
            if (!m) return;
            const key = map[m[1].toLowerCase()];
            if (!key) return;
            bump(key, parseInt(m[2], 10));
        });
    });
    return counters;
}
...
/**
 * Kernmerkmale für LF-Masken/Compile:
 * Class-Builder → Tab-1-State; CSC → Elternklasse (PHB coreData / Custom Class).
 */
function buildLfCoreTraitsStateFromParent(slug) {
    let core = null;
    if (typeof getCscParentCoreTraits === "function") {
        core = getCscParentCoreTraits();
    }
    if (!core && typeof classCoreTraitsList !== "undefined") {
        const key = String(slug || "").toLowerCase();
        core = classCoreTraitsList.find(c =>
            String(c.translationLabel || "").toLowerCase() === key
        ) || null;
    }
    if (!core) {
        return {
    ...
    return {
        savingThrowProficiencies: normalizeToArray(core.savingThrowProficiencies),
        weaponCategoryNumber: normalizeToArray(core.weaponCategoryNumber),
        weaponPropertyCategoryNumber: normalizeToArray(core.weaponPropertyCategoryNumber),
        armorCategoryNumber: normalizeToArray(core.armorCategoryNumber),
        skillCategoryNumber: normalizeToArray(core.skillCategoryNumber),
        toolLabel: core.toolLabel || 0
    };
}

function getActiveLfCoreTraitsState() {
    if (typeof isCustomSubclassEditorActive === "function" && isCustomSubclassEditorActive()) {
        const slug = (typeof getCscParentClassSlug === "function")
            ? getCscParentClassSlug()
            : (customSubclassEditorState?.targetClassSlug || "");
        return buildLfCoreTraitsStateFromParent(slug);
    }
    return customClassEditorState;
}
...
function compileCustomSubclassList(state, slug, translationsBag) {
    ...
    (state.subclasses || []).forEach((sc, i) => {
        ...
        // Leere UC-Beschreibung als "-" (kein Roh-Key als Fallback)
        putCompiledTranslation(
            translationsBag,
            descKey,
...
function compileCustomMagicSkillsData(state, slug, compileMeta) {
    ...
    let magicId = 0;
    let subSpellId = 0;
    // IDs nach Unterklasse namespaced (Basis=0 → 1…; UC 100 → 1000001…),
    // damit Class- und Subclass-Compile sowie Mehrfach-UCs nicht kollidieren.
    const nextMagicId = (subclassNum = 0) => {
        magicId += 1;
        return (Number(subclassNum) || 0) * 10000 + magicId;
    };
    const nextSubSpellId = (subclassNum = 0) => {
        subSpellId += 1;
        return (Number(subclassNum) || 0) * 10000 + subSpellId;
    };
    ...
    const pushSpellcastingMagic = (slot, subclassNum) => {
        ...
        const entry = createEmptyCompiledMagicSkillEntry(nextMagicId(subclassNum), slug, subclassNum);
    ...
    const pushSubclassSpellsFamily = (slots, subclassNum) => {
        ...
        const magicEntry = createEmptyCompiledMagicSkillEntry(nextMagicId(subclassNum), slug, subclassNum);
...
/**
 * Index-State aus constForChoice der Features (CSC: selectedExpertise5… statt 1…).
 */
function createCustomClassChoiceIndexStateFromFeatures(features) {
    ...
    const mins = {};
    (features || []).forEach(f => {
        const cfc = f?.constForChoice;
        if (typeof cfc !== "string") return;
        cfc.split(";").forEach(part => {
            const m = String(part).trim().match(/^selected([A-Za-z]+)(\d+)$/i);
            if (!m) return;
            const key = map[m[1].toLowerCase()];
            if (!key) return;
            ...
            if (mins[key] == null || n < mins[key]) mins[key] = n;
        });
    });
...
/**
 * Anzeigename einer kompilierten Merkmalszeile (Übersetzung oder Roh-Label).
 */
function resolveCcFeatureDisplayName(feature) {
    const elements = translations[currentLang] || {};
    if (feature?.translationLabel && feature.translationLabel !== 0) {
        return elements[feature.translationLabel] || String(feature.translationLabel);
    }
    return elements.chooseOptionLabel || "Option";
}

/**
 * Unterklassensektion (Schritt 6): „Merkmalsbezeichnung - Wähle eine Option:“
 * Ohne Kategorie-Zusatz (Fertigkeit/Werkzeug) – Bezeichnung und Kategorie sind oft gleich.
 */
function formatCcSubclassStep6DropdownLabel(feature) {
    const elements = translations[currentLang] || {};
    const name = resolveCcFeatureDisplayName(feature);
    const choose = elements.chooseOptionLabel || "Wähle eine Option";
    return `${name} - ${choose}:`;
}
...
function getGrantedCustomClassSkillCategoryNumbers() {
    if (!hasActiveCustomClassOrSubclassRuntime(character?.class)) {
        return [];
    }
...
/** true, wenn Custom-Klasse oder Standalone-Custom-UC für die aktuelle Klasse aktiv ist */
function hasActiveCustomClassOrSubclassRuntime(className) {
    const slug = className || character?.class;
    if (typeof isRegisteredCustomClassSlug === "function" && isRegisteredCustomClassSlug(slug)) {
        return true;
    }
    return typeof getRegisteredCustomSubclassBundle === "function"
        && !!getRegisteredCustomSubclassBundle(slug);
}
...
function renderCustomClassChoiceFeatures(container, features, indexState, renderOptions) {
    ...
    const isSubclassSection = opts.context === "subclass";
    ...
    /** Basisklasse: choose*-Label + Stufe; Unterklasse: Merkmalsname - Wähle eine Option */
    const labelFor = (feature, chooseKey, featLevel) => (isSubclassSection
        ? formatCcSubclassStep6DropdownLabel(feature)
        : formatCcStep6DropdownLabel(chooseKey, featLevel));
    ...
    features.forEach(feature => {
        ...
        if (feature.fightingStyleID && feature.fightingStyleID !== 0) {
            const optsHtml = createCustomFightingStyleOptionsHtml(feature.fightingStyleID);
            const labelText = labelFor(feature, "chooseFightingStyleLabel", featLevel);
        ...
        if (typeof feature.constForChoice === "string"
            ...
            const labelText = labelFor(feature, "chooseExpertiseLabel", featLevel);
            for (let i = 0; i < amount; i++) {
        ...
        if (typeof feature.constForChoice === "string"
            ...
            const optsHtml = typeof createWeaponOptions === "function"
                ? createWeaponOptions(wCats, wProps)
                : "";
            const labelText = labelFor(feature, "chooseWTLabel", featLevel);
            for (let i = 0; i < amount; i++) {
        ...
        if ((Array.isArray(feature.attributeID) && feature.attributeID.length)
            ...
            const optsHtml = typeof createSavingThrowOptions === "function"
                ? createSavingThrowOptions(feature.attributeID || [])
                : "";
            const labelText = labelFor(feature, "chooseSavingThrowLabel", featLevel);
            for (let i = 0; i < amount; i++) {
        ...
        if ((feature.maneuverCategoryNumber && feature.maneuverCategoryNumber !== 0)
            ...
            const optsHtml = typeof createManeuverOptions === "function"
                ? createManeuverOptions(feature.maneuverCategoryNumber || [])
                : "";
            const labelText = labelFor(feature, "chooseManeuverLabel", featLevel);
            for (let i = 0; i < amount; i++) {
        ...
        if ((feature.languageCategoryNumber && feature.languageCategoryNumber !== 0)
            ...
            const optsHtml = typeof createLangOptions === "function"
                ? createLangOptions(feature.languageCategoryNumber)
                : "";
            const labelText = labelFor(feature, "chooseLanguageLabel", featLevel);
            for (let i = 0; i < amount; i++) {
        ...
        if ((feature.toolCategoryNumber && feature.toolCategoryNumber !== 0)
            ...
            const labelText = labelFor(feature, parts.chooseKey, featLevel);
            for (let i = 0; i < amount; i++) {
        ...
        const fallbackText = isSubclassSection
            ? formatCcSubclassStep6DropdownLabel(feature)
            : (() => {
                ...
                return `${fallbackLabel} (${elements.levelLabel2 || "Stufe"} ${featLevel}):`;
            })();
        if (typeof feature.constForChoice === "string"
 ...
 * @param {"base"|"subclass"} [renderOptions.context]
 */
function renderCustomClassSkillChoiceDropdowns(container, feature, indexState, renderOptions) {
    ...
    const labelText = (renderOptions?.context === "subclass")
        ? formatCcSubclassStep6DropdownLabel(feature)
        : formatCcStep6DropdownLabel("chooseSkillLabel", featLevel);
...
function populateCustomClassSubclassChoiceSection(container, subclassFeatures, options) {
    ...
    const idx = opts.indexState
        || createCustomClassChoiceIndexStateFromFeatures(features);
...
function renderCustomClassAttributeDistributionUI(container, feature) {
    ...
    allowed.forEach(attrLabel => {
        ...
        // Schritt 6 mobil (≤600px): Abkürzungen, sonst Vollnamen
        const abbrKey = `${stringId}AbbrLabel`;
        const name = (window.innerWidth <= 600
            ? (elements[abbrKey] || elements[attrLabel])
            : elements[attrLabel]) || attrLabel;
        html += `
            <div class="asi-row">
...
function buildExportPayload(state, slug, id) {
    ...
    // Kein de↔en-Fallback: sonst holt eine bewusst geleerte Beschreibung
    // eine alte andere Sprache zurück (z. B. erstes EN-Wort „chemiker“).
    const descDe = String(state.descriptions?.de ?? "").trim();
    const descEn = String(state.descriptions?.en ?? "").trim();
...
function clearClassSelectionUI() {
    ...
    document.querySelectorAll('input[name="subclass"]').forEach(r => { r.checked = false; });
    const subclassOptionsDiv = document.getElementById("subclassOptions");
    if (subclassOptionsDiv) subclassOptionsDiv.innerHTML = "";
    if (typeof resetDynamicSubclassContent === "function") resetDynamicSubclassContent();
    if (typeof resetSubclassUI === "function") resetSubclassUI();
}
...
function registerCustomClassInRuntime(rawPackage) {
    ...
    // source für Legacy-Pakete ohne Tag nachziehen
    if (typeof applyCustomContentSource === "function") {
        applyCustomContentSource(core);
        (Array.isArray(payload.compiledSubclassList) ? payload.compiledSubclassList : [])
            .forEach(sc => applyCustomContentSource(sc));
    }
...
function ensureCustomClassTextNode(slug) {
    ...
    node.innerHTML = String(
        translations[currentLang][`${slug}DLabel`]
        ?? translations[currentLang][`${slug}Text`]
        ?? ""
    ).replace(/\n/g, "<br>");
}
...
function hydrateEditorStateFromExport(data, envelope) {
    ...
    // ?? statt || : leere Beschreibung nicht durch Text/andere Reste ersetzen
 ...
 * Identity/Equip-Limits: siehe CUSTOM_CLASS_CONFIG am Dateianfang.
 */
const CUSTOM_CLASS_LF_CONFIG = {
    ...
    preDefinedFeatureMeta: {
        // Ersetzt die getrennten PHB-Einträge (Barbar/Mönch) durch ein frei wählbares Attribut-Paar
        customUnarmoredDefense: {
            displayLabelKey: "unarmoredDefenseLabel",
            shortKey: "ccPreDefinedUnarmoredDefenseShortD",
            longKey: "ccPreDefinedUnarmoredDefenseD",
            requires: null,
            sheetFlag: 3,
            parameters: [],
            // Gesamt 2 Attribute: GES fest + genau 1 freies Attribut
            abilityPickCount: 2,
            fixedAbilities: Object.freeze(["dexterityLabel"]),
            freeAbilityPickCount: 1
        },
        extraAttackLabel: {
 ...
 * Identity/Equip-Limits der Klasse: siehe CUSTOM_CLASS_CONFIG am Dateianfang.
 * Standalone-UC (Schritt 6): siehe CUSTOM_SUBCLASS_CONFIG in subclassBuilder.js.
 */
const CUSTOM_CLASS_SC_CONFIG = {
    ...
    limits: {
        /**
         * Max. eindeutige Dynamic-Content-Kategorien (Optionen, inkl. Fertigkeiten)
         * pro Unterklasse. ASI+Talent ist in Unterklassen nicht wählbar.
         */
        optionsTypeMax: 3
    },
    /** In Tab-3-Merkmalen nicht wählbare Attribut-Kategorien */
    blockedAttributeCategories: ["distribution"],
    /** In Unterklassen-Merkmalen nicht anzeigbare Options-Kategorien */
    blockedOptionsCategories: ["asiAndFeat"]
};
...
/**
 * Voraussetzung für Zauberwissen→Unterklassenzauber:
 * – Standalone-UC: Elternklasse zaubert/Paktmagie ODER diese UC hat Einfach→Zauberwirken
 * – Class-Builder: Tab-2-Zauberwirken ODER Zauberwirken auf derselben Unterklasse
 * @param {object} [slot] optionaler Slot (für Tab-3-Scope)
 */
function hasLfSubclassSpellsSpellcastingPrerequisite(slot) {
    if (typeof isCustomSubclassEditorActive === "function" && isCustomSubclassEditorActive()) {
        if (typeof cscParentHasBaseSpellcasting === "function" && cscParentHasBaseSpellcasting()) {
            return true;
        }
        return typeof cscThisSubclassHasSpellcastingFeature === "function"
            && cscThisSubclassHasSpellcastingFeature();
    }
    if (hasLfBaseClassSpellcasting(customClassEditorState)) return true;
    const subclassId = slot?.subclassId;
...
/** Min. Stufe für Unterklassenzauber (Eingangsstufe Zauberwirken) */
function getLfSubclassSpellsPrerequisiteLevel(slot) {
    if (typeof isCustomSubclassEditorActive === "function" && isCustomSubclassEditorActive()) {
        if (typeof cscParentHasBaseSpellcasting === "function" && cscParentHasBaseSpellcasting()) {
            if (typeof getCscParentSpellcastingLevel === "function") {
                return getCscParentSpellcastingLevel() || 1;
            }
            return 1;
        }
        if (typeof getCscSpellcastingFeatureSlot === "function") {
            const spellSlot = getCscSpellcastingFeatureSlot();
            return spellSlot ? (Number(spellSlot.level) || 1) : null;
        }
        return null;
    }
    if (hasLfBaseClassSpellcasting(customClassEditorState)) {
        return getLfBaseSpellcastingLevel(customClassEditorState);
    }
    const subclassId = slot?.subclassId;
    if (!subclassId) return null;
    const sc = findCustomClassSubclassById(subclassId);
    const spellSlot = (sc?.levelFeatures || []).find(s =>
        s?.payload?.featureType === "simple" && s.payload?.category === "spellcasting"
    );
    return spellSlot ? (Number(spellSlot.level) || 1) : null;
}
...
/**
 * Effektive Zauberwirken-Stufe nach hypothetischem Payload-Tausch (für DnD-Prüfung).
 * @returns {number|null}
 */
function getLfSpellcastingLevelAfterPayloadSwap(slotA, slotB) {
    const payloadAt = (slot) => {
        if (slot.slotId === slotA.slotId) return slotB.payload;
        if (slot.slotId === slotB.slotId) return slotA.payload;
        return slot.payload;
    };
    const isSpell = (p) => p?.featureType === "simple" && p?.category === "spellcasting";

    if (typeof isCustomSubclassEditorActive === "function" && isCustomSubclassEditorActive()) {
        if (typeof cscParentHasBaseSpellcasting === "function" && cscParentHasBaseSpellcasting()) {
            return (typeof getCscParentSpellcastingLevel === "function")
                ? (getCscParentSpellcastingLevel() || 1)
                : 1;
        }
        const slots = (typeof getCustomSubclassLfBridge === "function"
            && getCustomSubclassLfBridge()?.slots)
            || [];
        const hit = slots.find(s => isSpell(payloadAt(s)));
        return hit ? (Number(hit.level) || 1) : null;
    }

    const classSlots = customClassEditorState?.levelFeatures || [];
    const classHit = classSlots.find(s =>
        s && !s.blockedBySubclass && isSpell(payloadAt(s))
    );
    if (classHit) return Number(classHit.level) || 1;

    const subclassId = slotA.subclassId || slotB.subclassId;
    if (subclassId) {
        const sc = findCustomClassSubclassById(subclassId);
        const hit = (sc?.levelFeatures || []).find(s => isSpell(payloadAt(s)));
        return hit ? (Number(hit.level) || 1) : null;
    }
    return null;
}

/** true, wenn Payload Unterklassenzauber ist */
function isLfSubclassSpellsPayload(payload) {
    return !!(payload
        && payload.featureType === "spellcraft"
        && payload.category === "subclassSpells");
}

/**
 * Unterklassenzauber darf nicht unter die Zauberwirken-Stufe geschoben werden.
 * Beim Verschieben von Zauberwirken: keine UC-Zauber unter der neuen Stufe hinterlassen.
 */
function canLfSwapRespectingSubclassSpells(slotA, slotB) {
    const minLvl = getLfSpellcastingLevelAfterPayloadSwap(slotA, slotB);
    const afterA = slotB.payload;
    const afterB = slotA.payload;

    if (minLvl == null) {
        // Ohne verbleibendes Zauberwirken: kein Unterklassenzauber nach dem Tausch
        if (isLfSubclassSpellsPayload(afterA) || isLfSubclassSpellsPayload(afterB)) {
            return false;
        }
        return true;
    }

    if (isLfSubclassSpellsPayload(afterA) && (Number(slotA.level) || 0) < minLvl) return false;
    if (isLfSubclassSpellsPayload(afterB) && (Number(slotB.level) || 0) < minLvl) return false;

    // Tab-2: Spellcasting-Stufe steigt → bestehende Tab-3-UC-Zauber unter neuer Stufe?
    if (!isLfSubclassFeatureSlot(slotA) && !isLfSubclassFeatureSlot(slotB)) {
        const invalid = (customClassEditorState.subclasses || []).some(sc =>
            (sc.levelFeatures || []).some(s =>
                isLfSubclassSpellsPayload(s.payload)
                && (Number(s.level) || 0) < minLvl
            )
        );
...
/**
 * Unterklassenzauber-Slots unter minLevel zurücksetzen (nach Spellcasting-Änderung).
 * minLevel = Infinity → alle Unterklassenzauber im Scope entfernen.
 * @returns {boolean} true wenn etwas geändert wurde
 */
function sanitizeLfSubclassSpellsBelowLevel(minLevel, scopeSlots) {
    if (minLevel == null) return false;
    const min = Number(minLevel);
    // Infinity ist erlaubt (alle löschen); NaN nicht
    if (Number.isNaN(min)) return false;
    let changed = false;
    const clear = (list) => {
        (list || []).forEach(s => {
            if (!isLfSubclassSpellsPayload(s.payload)) return;
            if (Number.isFinite(min) && (Number(s.level) || 0) >= min) return;
            s.payload.featureType = null;
            s.payload.category = null;
            s.payload.optionsConfig = null;
            s.payload.amount = null;
            s.payload.names = { de: "", en: "" };
            s.payload.shortDescriptions = { de: "", en: "" };
    ...
    if (scopeSlots) {
        clear(scopeSlots);
    } else if (typeof isCustomSubclassEditorActive === "function" && isCustomSubclassEditorActive()) {
        clear(customSubclassEditorState?.levelFeatures);
    } else {
        (customClassEditorState.subclasses || []).forEach(sc => clear(sc.levelFeatures));
    }
    return changed;
}

/** Alle Unterklassenzauber im Zauberwirken-Scope entfernen */
function clearLfSubclassSpellsInSpellcastingScope(slot, scopeSlots) {
    if (scopeSlots) {
        return sanitizeLfSubclassSpellsBelowLevel(Number.POSITIVE_INFINITY, scopeSlots);
    }
    if (typeof isCustomSubclassEditorActive === "function" && isCustomSubclassEditorActive()) {
        return sanitizeLfSubclassSpellsBelowLevel(Number.POSITIVE_INFINITY);
    }
    if (slot && isLfSubclassFeatureSlot(slot)) {
        const sc = findCustomClassSubclassById(slot.subclassId);
        return sanitizeLfSubclassSpellsBelowLevel(Number.POSITIVE_INFINITY, sc?.levelFeatures);
    }
    return sanitizeLfSubclassSpellsBelowLevel(Number.POSITIVE_INFINITY);
}

/** Ob im relevanten Scope Unterklassenzauber-Merkmale definiert sind */
function hasLfSubclassSpellsFeaturesInSpellcastingScope(slot) {
    const listHas = (list) => (list || []).some(s => isLfSubclassSpellsPayload(s?.payload));

    if (typeof isCustomSubclassEditorActive === "function" && isCustomSubclassEditorActive()) {
        return listHas(customSubclassEditorState?.levelFeatures);
    }
    if (isLfSubclassFeatureSlot(slot)) {
        const sc = findCustomClassSubclassById(slot.subclassId);
        return listHas(sc?.levelFeatures);
    }
    // Tab-2-Zauberwirken betrifft alle Unterklassen
    return (customClassEditorState.subclasses || []).some(sc => listHas(sc.levelFeatures));
}

/**
 * Warnung vor Entfernen von Zauberwirken, wenn Unterklassenzauber definiert sind.
...
function confirmRemoveSpellcastingClearsSubclassSpells(slot) {
    ...
    const msg = tCC(
        ...
        "Wenn du das Merkmal Zauberwirken entfernst, werden deine definierten Unterklassenzauber ebenfalls entfernt und nicht gespeichert. Trotzdem fortfahren?"
    );
    return confirm(msg);
}

/** Zauberlisten der Eingangsstufe (aktive Editor-Progression / Spellcasting-Slot) */
function getLfSpellcastingEntrySpellListLabels(state) {
    const prog = state?.spellcastingProgression || getActiveEditorSpellcastingProgression();
    if (Array.isArray(prog?.baseSpellListLabels) && prog.baseSpellListLabels.length) {
        return prog.baseSpellListLabels.filter(Boolean);
    }
    if (typeof isCustomSubclassEditorActive === "function" && isCustomSubclassEditorActive()
        && typeof getCscSpellcastingFeatureSlot === "function") {
        const cscSlot = getCscSpellcastingFeatureSlot();
        const lists = cscSlot?.payload?.optionsConfig?.spellListLabels;
        if (Array.isArray(lists) && lists.length) return lists.filter(Boolean);
        if (typeof getCscParentSpellListLabels === "function") {
            const parentLists = getCscParentSpellListLabels();
            if (Array.isArray(parentLists) && parentLists.length) return parentLists.filter(Boolean);
        }
    }
    const slot = getLfBaseSpellcastingSlot(state || customClassEditorState);
...
/** In Tab 4 / CSC-Zauber-Tab freigeschaltete Zaubergrade (inkl. Cantrips) */
function getLfTab4UnlockedSpellGradeLabels(state, level) {
    const prog = state?.spellcastingProgression
        || getActiveEditorSpellcastingProgression();
    const row = getSpellProgRowFromProgression(prog, level);
    if (!row) return [];
    const labels = [];
...
function isLfSubclassSpellsCategorySelectable(slot, slots) {
    ...
    if (!hasLfSubclassSpellsSpellcastingPrerequisite(slot)) return false;
    const baseLvl = getLfSubclassSpellsPrerequisiteLevel(slot);
...
function sanitizeLfSubclassSpellsSlotSelections(slot, slots, state) {
    ...
    const spellState = state
        || ((typeof isCustomSubclassEditorActive === "function"
            && isCustomSubclassEditorActive()
            && typeof customSubclassEditorState !== "undefined")
            ? customSubclassEditorState
            : customClassEditorState);
    const allowed = new Set(getLfSubclassSpellsAllowedGradeLabels(slot, slots, spellState));
    const next = {};
    let changed = false;
...
/** Attribut-Kürzel über Core-Keys (strengthAbbrLabel … charismaAbbrLabel) */
function getLfAbilityModAbbrLabel(abilityLabel, lang) {
    const abbrKeyMap = {
    ...
    const abbrKey = abbrKeyMap[abilityLabel];
    if (!abbrKey) return "";
    const resolvedLang = lang || (typeof currentLang !== "undefined" ? currentLang : "de");
    return resolveLfTranslationLabelText(abbrKey, resolvedLang);
}

/**
 * Text für "Ungerüstete Verteidigung" (Custom) mit den gewählten Attributen befüllen.
 * abilities = [abilityLabel1, abilityLabel2]; unvollständig → Platzhalter "…".
 */
function formatLfCustomUnarmoredDefenseDesc(abilities, lang, mode) {
    const key = mode === "short" ? "ccPreDefinedUnarmoredDefenseShortD" : "ccPreDefinedUnarmoredDefenseD";
    const tpl = (typeof translations !== "undefined" && translations[lang] && translations[lang][key])
        || (typeof translations !== "undefined" && translations.de && translations.de[key])
        || "";
    const a1 = abilities?.[0] ? (resolveLfTranslationLabelText(abilities[0], lang) || "…") : "…";
    const a2 = abilities?.[1] ? (resolveLfTranslationLabelText(abilities[1], lang) || "…") : "…";
    const m1 = abilities?.[0] ? (getLfAbilityModAbbrLabel(abilities[0], lang) || "…") : "…";
    const m2 = abilities?.[1] ? (getLfAbilityModAbbrLabel(abilities[1], lang) || "…") : "…";
    if (tpl) {
        return String(tpl)
            .replace(/\{ability1\}/g, a1)
            .replace(/\{ability2\}/g, a2)
            .replace(/\{mod1\}/g, m1)
            .replace(/\{mod2\}/g, m2);
    }
    // Fallback, falls Templates einmal fehlen sollten
    return mode === "short"
        ? (lang === "en"
            ? `Without armor or a shield, your ${a1} and ${a2} protect you.`
            : `Ohne Rüstung und Schild schützt dich deine ${a1} und ${a2}.`)
        : (lang === "en"
            ? `While unarmored and without a shield: AC = 10 + ${m1} modifier + ${m2} modifier.`
            : `Ohne Rüstung und Schild: RK = 10 + ${m1}-Modifikator + ${m2}-Modifikator.`);
}
...
/**
 * Alias-Gruppen für Vordefiniert-Labels (PHB nutzt teils ohne „Label“-Suffix).
 * PHB unarmoredDefense (Barbar) / unarmoredDefenseLabel (Mönch) sind im Ersteller nicht
 * mehr wählbar, zählen aber weiter als „bereits vorhanden“ (Elternklassen-/Slot-Erkennung).
 */
function getLfPreDefinedLabelAliasGroup(label) {
    const groups = [
        ["extraAttack", "extraAttackLabel"],
        ["twoExtraAttacks", "twoExtraAttacksLabel"],
        ["threeExtraAttacks", "threeExtraAttacksLabel"],
        // Custom Unarmored Defense ersetzt die PHB-Einträge, zählt aber weiter als „bereits vorhanden“
        ["customUnarmoredDefense", "unarmoredDefense", "unarmoredDefenseLabel"]
    ];
    for (let i = 0; i < groups.length; i++) {
        if (groups[i].includes(label)) return groups[i].slice();
    }
    return label ? [label] : [];
}

function lfPreDefinedLabelsMatch(a, b) {
    if (!a || !b) return false;
    if (a === b) return true;
    const group = getLfPreDefinedLabelAliasGroup(a);
    return group.includes(b);
}

/** Basisklassen-Zeilen der CSC-Elternklasse (PHB oder registrierte Custom Class) */
function getCscParentBaseClassDataRows() {
    if (typeof isCustomSubclassEditorActive !== "function" || !isCustomSubclassEditorActive()) {
        return [];
    }
    const slug = (typeof getCscParentClassSlug === "function")
    ...
    return getClassData(String(slug).toLowerCase(), "class") || [];
}

/** Ob ClassData-Zeilen (nur Basis) ein Vordefiniert-Label tragen */
function classDataRowsHavePreDefinedLabel(rows, label) {
    if (!label) return false;
    const matchSet = new Set(getLfPreDefinedLabelAliasGroup(label));
    return (rows || []).some(r => {
        if (!r?.translationLabel || r.translationLabel === 0) return false;
        if ((r.subclassCategoryNumber || 0) > 0) return false;
        return matchSet.has(r.translationLabel);
    });
}

/** CSC: Elternklasse besitzt dieses Vordefiniert-Merkmal bereits */
function cscParentHasPreDefinedLabel(label) {
    if (typeof isCustomSubclassEditorActive !== "function" || !isCustomSubclassEditorActive()) {
        return false;
    }
    return classDataRowsHavePreDefinedLabel(getCscParentBaseClassDataRows(), label);
}
...
function isLfPreDefinedLabelUsedInSlots(slots, label, excludeSlotId) {
    return (slots || []).some(s =>
        ...
        && lfPreDefinedLabelsMatch(s.payload?.optionsConfig?.preDefinedLabel, label)
    );
}
 ...
 * CSC: zusätzlich gesperrt, wenn die Elternklasse das Merkmal schon hat.
 */
function canSelectLfPreDefinedLabel(slot, label) {
    ...
    if (lfPreDefinedLabelsMatch(current, label)) return true;
    if (!isLfPreDefinedOnceOnly(label)) return true;

    // Standalone-UC: Elternklasse (PHB/Custom) zählt als „bereits vorhanden“
    if (typeof isCustomSubclassEditorActive === "function" && isCustomSubclassEditorActive()) {
        if (cscParentHasPreDefinedLabel(label)) return false;
...
function formatLfFreeOptionsCopySourceLabel(slot, sources) {
    ...
    const scTitle = getSubclassDisplayName(sc)
        || `${tCC("subclass")} ${sc?.subclassCategoryNumber || ""}`.trim();
...
function syncLfDescLongFromDom(slot, forceTokenEnabled) {
    ...
    const max = CUSTOM_CLASS_LF_CONFIG.descMax || 500;
    ensureAvailableLanguages(getActiveLfEditorLangState()).forEach(lang => {
...
function refreshLfFloatDescBodyFromSlot(slot) {
    const bodyEl = getActiveLfFloatBodyEl();
...
function buildLfLongDescSectionHtml(slot) {
    ...
    return `
        <div class="cc-lf-desc-section">
            <div class="cc-lf-float-field-label">${tCC("descriptionLabel")}</div>
...
function hasLfPreDefinedOnEarlierLevel(slots, label, level, excludeSlotId, index = Number.POSITIVE_INFINITY) {
    ...
    return (slots || []).some(s => {
        ...
        return lfPreDefinedLabelsMatch(stored, label);
    });
}

/**
 * Vordefiniert-Voraussetzung erfüllt?
 * Frühere UC-/Klassen-Slots ODER (nur CSC) Merkmal bereits in der Elternklasse.
 */
function hasLfPreDefinedPrerequisite(slot, requiresLabel) {
    if (!requiresLabel || !slot) return true;
    const slots = getLfSlotsForSlot(slot);
    if (hasLfPreDefinedOnEarlierLevel(
        slots, requiresLabel, slot.level, slot.slotId, slot.index
    )) {
        return true;
    }
    // Append-Kette: z. B. Extra Attack der Elternklasse → Zwei Extra Attacks wählbar
    if (typeof isCustomSubclassEditorActive === "function" && isCustomSubclassEditorActive()) {
        return cscParentHasPreDefinedLabel(requiresLabel);
    }
    return false;
}
...
function getLfAllowedCategoriesForSlot(slot, type) {
    ...
    if (type === "options" && isLfSubclassFeatureSlot(slot)) {
        const blocked = CUSTOM_CLASS_SC_CONFIG.blockedOptionsCategories || [];
        cats = cats.filter(c => !blocked.includes(c));
    }
    if (type === "spellcraft") {
        // Unterklassenzauber nur in UC; Kategorie immer listen (Sperre via disabled),
        // damit Voraussetzung sichtbar bleibt
        if (!isLfSubclassFeatureSlot(slot)) {
...
function buildLfSpellcraftFilterBlockHtml({
    ...
    const extraCall = onchangeExtra
        ? onchangeExtra.replace(/;?\s*$/, "")
        : "";
    const gridOpts = checkboxOnchange ? { onchange: checkboxOnchange } : {};
    return `
        ${buildLfControlRowHtml(
            ...
            `<select id="${modeId}" class="dropdown cc-lf-float-input cc-lf-options-filter"
                onchange="onLfSpellcraftFilterModeChange(this, '${listId}'${extraCall ? `, function(){ ${extraCall}; }` : ""})">
                <option value="all" ${mode === "all" ? "selected" : ""}>${tCC("allLabel")}</option>
                <option value="selection" ${mode === "selection" ? "selected" : ""}>${tCC("selectionLabel")}</option>
            </select>`
        )}
...
/**
 * Filtermodus Alle/Auswahl: Checkbox-Liste im aktiven Float ein-/ausblenden.
 * @param {HTMLSelectElement} selectEl
 * @param {string} listId – z. B. ccLfScSchools
 * @param {Function} [extraFn] – z. B. softRebuild…
 */
function onLfSpellcraftFilterModeChange(selectEl, listId, extraFn) {
    const body = selectEl?.closest?.(".cc-lf-float-body") || getActiveLfFloatBodyEl();
    const wrap = body?.querySelector?.(`#${listId}Wrap`)
        || queryActiveLfFloat(`#${listId}Wrap`);
    if (wrap) {
        wrap.style.display = selectEl.value === "selection" ? "block" : "none";
    }
...
/**
 * Halbzauberer-Tabelle (Eldritch Knight / Arcane Trickster, PHB 2024).
 * Index = Charakterstufe − startLevel (bei Start 3: Index 16/17 = Stufe 19/20 mit Grad 4).
 */
const HALF_CASTER_SPELL_PROGRESSION_TABLE = {
    0: { c: 2, p: 3, s: [2, 0, 0, 0, 0, 0, 0, 0, 0] },
    1: { c: 2, p: 4, s: [3, 0, 0, 0, 0, 0, 0, 0, 0] },
    2: { c: 2, p: 4, s: [3, 0, 0, 0, 0, 0, 0, 0, 0] },
    3: { c: 2, p: 4, s: [3, 0, 0, 0, 0, 0, 0, 0, 0] },
    4: { c: 2, p: 5, s: [4, 2, 0, 0, 0, 0, 0, 0, 0] },
    5: { c: 2, p: 6, s: [4, 2, 0, 0, 0, 0, 0, 0, 0] },
    6: { c: 2, p: 6, s: [4, 2, 0, 0, 0, 0, 0, 0, 0] },
    7: { c: 2, p: 7, s: [4, 3, 0, 0, 0, 0, 0, 0, 0] },
    8: { c: 3, p: 8, s: [4, 3, 0, 0, 0, 0, 0, 0, 0] },
    9: { c: 3, p: 9, s: [4, 3, 2, 0, 0, 0, 0, 0, 0] },
    10: { c: 3, p: 10, s: [4, 3, 2, 0, 0, 0, 0, 0, 0] },
    11: { c: 3, p: 10, s: [4, 3, 2, 0, 0, 0, 0, 0, 0] },
    12: { c: 3, p: 11, s: [4, 3, 3, 0, 0, 0, 0, 0, 0] },
    13: { c: 3, p: 11, s: [4, 3, 3, 0, 0, 0, 0, 0, 0] },
    14: { c: 3, p: 12, s: [4, 3, 3, 0, 0, 0, 0, 0, 0] },
    15: { c: 3, p: 12, s: [4, 3, 3, 0, 0, 0, 0, 0, 0] },
    16: { c: 3, p: 13, s: [4, 3, 3, 1, 0, 0, 0, 0, 0] },
    17: { c: 3, p: 13, s: [4, 3, 3, 1, 0, 0, 0, 0, 0] }
};

/** Eine Halbzauberer-Zeile für eine Charakterstufe */
function buildHalfCasterSpellProgRow(characterLevel, startLevel, baseLists) {
    const start = Number(startLevel) || 1;
    const idx = Math.min(17, Math.max(0, (Number(characterLevel) || start) - start));
    const t = HALF_CASTER_SPELL_PROGRESSION_TABLE[idx] || HALF_CASTER_SPELL_PROGRESSION_TABLE[17];
    const row = createEmptySpellProgRow(baseLists || []);
    row.cantripsAmount = t.c;
    row.preparedSpellsAmount = t.p;
    for (let i = 1; i <= 9; i++) row[`SSpSL${i}`] = t.s[i - 1] || 0;
    return row;
}

/**
 * Volle Halbzauberer-Progression als userRows (CSC-Standard / Tab-4 User-Modus).
 */
function buildHalfCasterSpellProgFromTemplate(startLevel, baseLists) {
    const start = Number(startLevel) || 1;
    ...
    for (let lvl = start; lvl <= 20; lvl++) {
        prog.userRows[String(lvl)] = buildHalfCasterSpellProgRow(lvl, start, lists);
    }
    return prog;
}
...
function getActiveSpellProgRow(level) {
    const prog = (typeof getActiveEditorSpellcastingProgression === "function")
        ? getActiveEditorSpellcastingProgression()
        : customClassEditorState?.spellcastingProgression;
...
/**
 * Aktive Zauberprogression: Custom-Class-Tab-4 oder Standalone-UC-Builder.
 */
function getActiveEditorSpellcastingProgression() {
    if (typeof isCustomSubclassEditorActive === "function" && isCustomSubclassEditorActive()
        && typeof customSubclassEditorState !== "undefined"
        && customSubclassEditorState?.spellcastingProgression) {
        return customSubclassEditorState.spellcastingProgression;
    }
    return customClassEditorState?.spellcastingProgression;
}

function rerenderActiveSpellProgOwner() {
    if (typeof isCustomSubclassEditorActive === "function" && isCustomSubclassEditorActive()) {
 ...
 * User: userRows.
 * Standard: startLevel 1 → Vollzauberer (Wizard); startLevel > 1 → Halbzauberer (EK/AT).
 */
function getSpellProgRowFromProgression(prog, level) {
    ...
    // Unterklassen-Caster ab Stufe 3 u. ä.: PHB-Halbzauberer, nicht Wizard-Stufenverschiebung
    if (start > 1) {
        return buildHalfCasterSpellProgRow(level, start, p.baseSpellListLabels || []);
    }
...
function enforceSpellProgMonotonicityFrom(level) {
    const prog = getActiveEditorSpellcastingProgression();
...
/** Slot anhand ID in Tab-2- oder Tab-3-Slots finden (+ Standalone-UC-Builder) */
function resolveLfSlotContext(slotId) {
    ...
    // Standalone Custom Subclass Builder
    if (typeof getCustomSubclassLfBridge === "function") {
        const bridge = getCustomSubclassLfBridge();
        if (bridge?.slots) {
            slot = bridge.slots.find(s => s.slotId === slotId);
            if (slot) {
                return {
                    slot,
                    slots: bridge.slots,
                    subclass: bridge.subclassShim,
                    isSubclass: true
                };
            }
        }
...
function getLfSlotsForSlot(slot) {
    if (!slot) {
        if (typeof getCustomSubclassLfBridge === "function") {
            const bridge = getCustomSubclassLfBridge();
            if (bridge?.active) return bridge.slots || [];
        }
    ...
    if (slot.subclassId) {
        if (typeof getCustomSubclassLfBridge === "function") {
            const bridge = getCustomSubclassLfBridge();
            if (bridge?.active && bridge.subclassShim?.id === slot.subclassId) {
                return bridge.slots || [];
            }
        }
        const sc = findCustomClassSubclassById(slot.subclassId);
...
/**
 * Scrollposition der offenen Editor-Modals / Tabellen vor Re-Render sichern.
 * Speichert Selektor + Offset und setzt nach DOM-Replace erneut (alte Knoten sind disconnected).
 */
function runWithPreservedEditorScroll(fn) {
    const snaps = [];
    const pushSnap = (el, selector) => {
    ...
    const pushBySelector = (selector) => {
        pushSnap(document.querySelector(selector), selector);
    };

    const ccOverlay = document.getElementById("customClassOverlay");
    const cscOverlay = document.getElementById("customSubclassOverlay");
    const ccVisible = ccOverlay && window.getComputedStyle(ccOverlay).display !== "none";
    const cscVisible = cscOverlay && window.getComputedStyle(cscOverlay).display !== "none";

    if (ccVisible) {
        pushSnap(ccOverlay, null);
        pushSnap(ccOverlay.querySelector(".custom-class-modal"), "#customClassOverlay .custom-class-modal");
        pushBySelector("#customClassTab2Content .cc-lf-table-wrap");
        pushBySelector("#customClassTab3Content .cc-lf-table-wrap");
        pushBySelector("#customClassTab3Content .cc-sc-table-wrap");
        document.querySelectorAll("#customClassTab3Content .cc-sc-box-body").forEach((el, i) => {
            // Mehrere UC-Boxen: per Index wiederfinden
            snaps.push({
                selector: null,
                elId: null,
                indexSelector: "#customClassTab3Content .cc-sc-box-body",
                index: i,
                top: el.scrollTop,
                left: el.scrollLeft
            });
    ...
    if (cscVisible) {
        pushSnap(cscOverlay, null);
        pushSnap(cscOverlay.querySelector(".custom-class-modal"), "#customSubclassOverlay .custom-class-modal");
        pushBySelector("#customSubclassTab1Content .cc-lf-table-wrap");
        pushBySelector("#customSubclassTab1Content .cc-sc-table-wrap");
        pushBySelector("#customSubclassTab1Content .cc-sc-box-body");
    }

    const result = fn();
    const resolveEl = (snap) => {
        if (snap.elId) return document.getElementById(snap.elId);
        if (snap.indexSelector != null && snap.index != null) {
            return document.querySelectorAll(snap.indexSelector)[snap.index] || null;
        }
        if (snap.selector) return document.querySelector(snap.selector);
        return null;
    };
    const restore = () => {
        snaps.forEach(snap => {
            const el = resolveEl(snap);
            if (!el) return;
...
function rerenderLfOwner(ctx) {
    if (typeof isCustomSubclassEditorActive === "function" && isCustomSubclassEditorActive()) {
        if (typeof renderCustomSubclassTab1 === "function") renderCustomSubclassTab1();
        if (typeof updateCustomSubclassTab2Ui === "function") updateCustomSubclassTab2Ui();
        return;
    }
    if (ctx?.isSubclass) {
...
function isLfSimpleSpellcastingBlockedForSlot(slot, slots) {
    // Standalone-UC-Builder: eigene Regel (Eltern Full-Caster → blockiert; sonst max. 1× Spellcasting)
    if (typeof isCustomSubclassEditorActive === "function" && isCustomSubclassEditorActive()) {
        if (typeof cscParentIsFullCaster === "function" && cscParentIsFullCaster()) return true;
        const list = slots || getLfSlotsForSlot(slot) || [];
        return list.some(s =>
            s
 ...
 *  Zusätzlich: Unterklassenzauber nicht unter Zauberwirken-Stufe. */
function canLfSwapRespectingSubclassLevel(slotA, slotB) {
    // Unterklassenzauber ↔ Zauberwirken-Stufe (Tab 2/3 und CSC)
    if (!canLfSwapRespectingSubclassSpells(slotA, slotB)) return false;

    // Tab 3 / CSC: keine festen ASI/Subclass-Stufenregeln – nur Spell-Regel oben
...
/**
 * Dynamic-Content in Schritt 6:
 * - Basisklasse: Options außer Fertigkeiten / ASI+Talent (+ Attribut→Verteilung)
 * - Unterklasse: Options inkl. Fertigkeiten (ASI+Talent nicht wählbar); Limit optionsTypeMax
...
function getLfDynamicContentKey(slot) {
    ...
    const cat = slot.payload.category;
    if (!cat || cat === "asiAndFeat") return null;
    // Fertigkeiten: Basisklasse → Skill-Section (zählt nicht); Unterklasse → zählt mit
    if (cat === "skills" && !isLfSubclassFeatureSlot(slot)) return null;
    return `options:${cat}`;
}
...
/**
 * Tab 2: Fertigkeiten & ASI+Talent zählen nicht zum Dynamic-Content-Limit.
 * Unterklassen: keine Exemptions (ASI ohnehin ausgeblendet; Fertigkeiten zählen).
 */
function isLfOptionsCategoryExemptFromDynamicLimit(category, slot) {
    if (isLfSubclassFeatureSlot(slot)) return false;
 ...
 * (+ Fertigkeiten / ASI+Talent nur in Tab 2 ausgenommen).
 * Basisklasse: max. 4 · Unterklasse: max. 3 (CUSTOM_CLASS_*_CONFIG.limits).
 */
function canSelectLfDynamicContentCategory(slot, type, category, scopeSlots) {
    ...
    if (type === "options") {
        if (isLfOptionsCategoryExemptFromDynamicLimit(category, slot)) return true;
...
function getLfCategoryLabelKey(category) {
    const map = {
        ...
        skills: "skillsLabel",
        savingThrows: "savingThrowsLabel",
        weaponTraining: "weaponTrainingLabel",
        armorTraining: "armorTrainingLabel",
        expertise: "expertiseLabel",
        weaponMasteries: "weaponMasteriesLabel",
        tools: "toolsLabel",
...
function buildLfCategorySelectHtml(slot) {
    ...
    let html = `<select class="dropdown cc-lf-select" aria-label="${tCC("categoryLabel")}"
...
function onLfFeatureTypeChange(slotId, typeValue) {
    ...
    const wasSpellcasting = prevType === "simple" && slot.payload.category === "spellcasting";
    const removingSpellcasting = wasSpellcasting && nextType !== "simple";
    if (removingSpellcasting && !confirmRemoveSpellcastingClearsSubclassSpells(slot)) {
        rerenderLfOwner(ctx);
        return;
    }
...
function onLfCategoryChange(slotId, categoryValue) {
    ...
    const prevCatEarly = slot.payload.category;
    const removingSpellcasting = type === "simple"
        && prevCatEarly === "spellcasting"
        && nextCat !== "spellcasting";
    if (removingSpellcasting && !confirmRemoveSpellcastingClearsSubclassSpells(slot)) {
        rerenderLfOwner(ctx);
        return;
    }
    ...
    if (type === "spellcraft" && nextCat === "subclassSpells") {
        if (!isLfSubclassFeatureSlot(slot) || !hasLfSubclassSpellsSpellcastingPrerequisite(slot)) {
    ...
    // Unterklassenzauber bereinigen: bei Entfernen von Zauberwirken alles; sonst unter neuer Stufe
    if (type === "simple" && (nextCat === "spellcasting" || prevCat === "spellcasting")) {
        if (prevCat === "spellcasting" && nextCat !== "spellcasting") {
            ...
            if (minLvl != null) {
                if (isSubclass) sanitizeLfSubclassSpellsBelowLevel(minLvl, slots);
                else sanitizeLfSubclassSpellsBelowLevel(minLvl);
            }
        }
    }
...
function formatLfDesignation(slot) {
    ...
    if (slot.systemPreset === "subclass") return tCC("subclass", "Subclass");
        ...
        if (spec) {
            ...
            if (spec.designation === "tab1Tool") {
                const toolLabel = getTab1SurfaceToolLabel(customClassEditorState)
                    || slot.payload.optionsConfig?.toolSurfaceLabel;
                return toolLabel ? tCC(toolLabel, toolLabel) : tCC("toolsLabel", "Tools");
            }
...
/** Hinweis „Auswahl (genau N)“ */
function formatLfPickExactHint(n) {
    return String(tCC("ccLfPickExactHintLabel") || "")
        .replace(/\{n\}/g, String(n));
}

/**
 * Vordefiniert mit fixedAbilities: feste Attribute erzwingen und freie Auswahl ergänzen.
 * Reihenfolge: feste zuerst, dann übrige (DOM-/Nutzerreihenfolge).
 */
function normalizeLfPreDefinedSelectedAbilities(selected, meta) {
    const fixed = Array.isArray(meta?.fixedAbilities) ? meta.fixedAbilities.filter(Boolean) : [];
    const freeMax = meta?.freeAbilityPickCount != null
        ? meta.freeAbilityPickCount
        : Math.max(0, (meta?.abilityPickCount || 0) - fixed.length);
    const total = meta?.abilityPickCount || (fixed.length + freeMax);
    const incoming = Array.isArray(selected) ? selected.filter(Boolean) : [];
    const free = [];
    incoming.forEach(lab => {
        if (fixed.includes(lab)) return;
        if (free.includes(lab)) return;
        if (free.length < freeMax) free.push(lab);
    });
    return fixed.concat(free).slice(0, total);
}
...
/**
 * Anzeigenamen der Zauberlisten (z. B. wizard → Magier).
 */
function formatLfSpellListNamesForDesc(labels, lang) {
 ...
 * spellListLabels:
 *   – Array → feste Namen (Masken-Vorschau)
 *   – null/undefined → [LEVEL_VAL]spellListLabels[/LEVEL_VAL] (Charakterbogen, stufenabhängig)
 */
function formatLfSpellcastingCustomDesc(abilityLabel, focusLabels, lang, usesSpellbook, spellListLabels) {
    ...
    const focusText = focuses.length
        ...
        : noneText;
    // Compile: LEVEL_VAL bleibt stehen → Bogen ersetzt anhand Zauberprogression der Stufe
    const listsText = (spellListLabels === undefined || spellListLabels === null)
        ? "[LEVEL_VAL]spellListLabels[/LEVEL_VAL]"
        : formatLfSpellListNamesForDesc(spellListLabels, lang);
    if (!tpl) {
        const prepareLine = usesSpellbook
            ...
            : (lang === "en"
                ? `<li><b>Learning and changing prepared spells:</b> After finishing a Long Rest, you can change your prepared spells from your class spell lists (${listsText}).</li>`
                : `<li><b>Vorbereitete Zauber lernen und ändern:</b> Nach einer Langen Rast kannst du deine vorbereiteten Zauber aus den Zauberlisten (${listsText}) deiner Klasse anpassen.</li>`);
        return lang === "en"
    ...
    return String(tpl)
        ...
        .replace(/\{spellLists\}/g, listsText);
}
...
function canOpenLfOptionsMask(slot) {
    ...
    // Einfach → Vordefiniert: Maske nur wenn Merkmal gewählt und Parameter-Schema (oder Attribut-Wahl) vorhanden
    if (slot.payload.featureType === "simple" && slot.payload.category === "preDefined") {
        ...
        if (!label) return false;
        if (getLfPreDefinedFeatureMeta(label)?.abilityPickCount) return true;
        return getLfPreDefinedParameters(label).length > 0;
    }
...
function isLfOptionsConfigured(slot) {
    ...
    if (type === "simple") {
        if (cat === "preDefined") {
            ...
            if (!label) return false;
            const meta = getLfPreDefinedFeatureMeta(label);
            if (meta?.abilityPickCount) {
                const sel = normalizeLfPreDefinedSelectedAbilities(cfg.selectedAbilities, meta);
                return sel.length === meta.abilityPickCount && new Set(sel).size === sel.length;
            }
        ...
        if (cat === "weaponTraining") {
            ...
            const catMode = cfg.weaponCategoryMode || "selection";
            const propMode = cfg.weaponPropertyMode || "selection";
            const remainingCats = getLfRemainingWeaponCategoryNumbers(getActiveLfCoreTraitsState());
            const remainingProps = getLfRemainingWeaponPropertyCategoryNumbers(getActiveLfCoreTraitsState());
        ...
        if (cat === "armorTraining") {
            ...
            if (cfg.mode === "all" && getLfRemainingArmorCategoryNumbers(getActiveLfCoreTraitsState()).length) return true;
...
function getLfSkillsPoolCount(skillFilter) {
    ...
    if (skillFilter === "base") {
        ...
        const baseNums = new Set(
            normalizeToArray(core.skillCategoryNumber).map(n => parseInt(n, 10))
        );
...
function getLfSavingThrowPoolCount() {
    const granted = new Set(normalizeToArray(getActiveLfCoreTraitsState().savingThrowProficiencies));
    const attrs = [
        "strengthLabel", "dexterityLabel", "constitutionLabel",
        "intelligenceLabel", "wisdomLabel", "charismaLabel"
    ];
...
function countLfOptionsEntries(slot) {
    ...
    if (slot.payload.featureType === "simple") {
        ...
        if (slot.payload.category === "weaponTraining") {
            ...
            const propMode = cfg.weaponPropertyMode || "selection";
            let n = 0;
            if (catMode === "all") n += getLfRemainingWeaponCategoryNumbers(getActiveLfCoreTraitsState()).length;
            else n += (cfg.selectedWeaponCategoryNumbers || []).length;
            if (propMode === "all") n += getLfRemainingWeaponPropertyCategoryNumbers(getActiveLfCoreTraitsState()).length;
            else n += (cfg.selectedWeaponPropertyCategoryNumbers || []).length;
            return n;
        }
        if (slot.payload.category === "armorTraining") {
            if (cfg.mode === "all") return getLfRemainingArmorCategoryNumbers(getActiveLfCoreTraitsState()).length;
    ...
    if (isSc) {
        ...
        if (presetKeys) {
            ...
            return `<button type="button" class="cc-lf-chips cc-lf-chips--readonly" onclick="openLfFloat('${slot.slotId}', 'desc', event)"
                ...
                ${getLfLockIconHtml()}
                <span class="cc-lf-chip cc-lf-chip--on" title="${tCC("descriptionLabel")}">${letters.long}</span>
            </button>`;
        }
        ...
        return `<button type="button" class="cc-lf-chips" onclick="openLfFloat('${slot.slotId}', 'desc', event)"
            ...
            <span class="cc-lf-chip${hasL ? " cc-lf-chip--on" : ""}" title="${tCC("descriptionLabel")}">${letters.long}</span>
        </button>`;
    }
...
function buildLfOptionsChipHtml(slot) {
    ...
    if (slot.payload.featureType === "simple" && slot.payload.category === "preDefined") {
        const cfg = slot.payload.optionsConfig || {};
        // Custom Unarmored Defense: nur freies Attribut-Kürzel (GES fest → nicht im Chip)
        if (getLfPreDefinedFeatureMeta(cfg.preDefinedLabel)?.abilityPickCount) {
            const meta = getLfPreDefinedFeatureMeta(cfg.preDefinedLabel);
            const fixed = new Set(Array.isArray(meta?.fixedAbilities) ? meta.fixedAbilities : []);
            const free = (Array.isArray(cfg.selectedAbilities) ? cfg.selectedAbilities : [])
                .filter(a => a && !fixed.has(a));
            const abbrLang = getActiveUiLang();
            const label = on && free.length
                ? `${icon} ${free.map(a => escapeLfHtml(getLfAbilityModAbbrLabel(a, abbrLang))).join(" + ")}`
                : `${icon} —`;
            return `<button type="button" class="cc-lf-chip-btn${on ? " cc-lf-chip-btn--on" : ""}"
                onclick="openLfFloat('${slot.slotId}', 'options', event)"
                onmousedown="event.stopPropagation()" title="${tCC("ccLfEditOptionsTitleLabel")}">${label}</button>`;
...
function renderLfFloatLangFields(prefix, values, maxLen, isTextarea) {
    const activeLang = getActiveUiLang();
    const available = ensureAvailableLanguages(getActiveLfEditorLangState());
...
function buildLfSimplePreDefinedSelectHtml(slot) {
    ...
    const opts = CUSTOM_CLASS_LF_CONFIG.preDefinedFeatures.map(label => {
        ...
        const unlocked = !requires || hasLfPreDefinedPrerequisite(slot, requires);
        const onceOk = canSelectLfPreDefinedLabel(slot, label);
        const selected = lfPreDefinedLabelsMatch(current, label) ? "selected" : "";
        const disabled = ((!unlocked || !onceOk) && !lfPreDefinedLabelsMatch(current, label))
            ? "disabled"
            : "";
        const reqHint = (!unlocked && requires)
            ? ` (${tCC("ccLfRequiresEarlierLabel")}: ${tCC(requires)})`
            : "";
        const displayText = tCC(meta?.displayLabelKey || label, label);
        return `<option value="${label}" ${selected} ${disabled}>${displayText}${reqHint}</option>`;
    }).join("");
    return `
...
function buildLfFixedDescPreviewBlock(labelKey, translationKey, previewOpts) {
    ...
    if (translationKey === "ccPreDefinedUnarmoredDefenseShortD" || translationKey === "ccPreDefinedUnarmoredDefenseD") {
        const slot = previewOpts?.slot;
        const abilities = slot?.payload?.optionsConfig?.selectedAbilities || [];
        const mode = translationKey === "ccPreDefinedUnarmoredDefenseShortD" ? "short" : "long";
    ...
    if (translationKey === "spellcastingCustomD" || translationKey === "spellcastingCustomSpellbookD") {
        const slot = previewOpts?.slot;
        const cfg = slot?.payload?.optionsConfig || {};
        const state = (typeof isCustomSubclassEditorActive === "function" && isCustomSubclassEditorActive()
            && typeof customSubclassEditorState !== "undefined")
            ? customSubclassEditorState
            : (typeof customClassEditorState !== "undefined" ? customClassEditorState : null);
        const ability = cfg.spellcastingAbility || state?.spellcastingAbility || 0;
        const focusRaw = cfg.spellcastingFocus != null ? cfg.spellcastingFocus : state?.spellcastingFocus;
        const focuses = typeof normalizeToArray === "function"
            ? normalizeToArray(focusRaw).filter(Boolean)
            : (Array.isArray(focusRaw) ? focusRaw.filter(Boolean) : []);
        const usesBook = !(slot && isLfSubclassFeatureSlot(slot)) && focuses.includes("spellbookLabel");
        // Vorschau: Listen aus Merkmalsmaske / Progressionseingang (nicht LEVEL_VAL)
        const lists = Array.isArray(cfg.spellListLabels) && cfg.spellListLabels.length
            ? cfg.spellListLabels
            : (typeof getActiveEditorSpellcastingProgression === "function"
                ? (getActiveEditorSpellcastingProgression()?.baseSpellListLabels || [])
                : []);
        pair.de = formatLfSpellcastingCustomDesc(ability, focuses, "de", usesBook, lists);
        pair.en = formatLfSpellcastingCustomDesc(ability, focuses, "en", usesBook, lists);
    }
    const activeLang = getActiveUiLang();
    const available = ensureAvailableLanguages(getActiveLfEditorLangState());
    const ordered = [activeLang, ...available.filter(l => l !== activeLang)];
...
function buildLfFloatDescBody(slot) {
    if (isLfDescMaskReadonly(slot)) {
        ...
        if (isLfSubclassFeatureSlot(slot)) {
            const longHtml = keys.longKey
                ? buildLfFixedDescPreviewBlock("descriptionLabel", keys.longKey, { slot })
                : "";
            return longHtml || `<p>—</p>`;
        }
        const shortHtml = keys.shortKey
            ? buildLfFixedDescPreviewBlock("ccLfChipShortLabel", keys.shortKey, { slot })
            : "";
        const longHtml = keys.longKey
            ? buildLfFixedDescPreviewBlock("descriptionLabel", keys.longKey, { slot })
            : "";
...
function buildLfCheckboxGridHtml(listId, items, selectedSet, max = null, displayOpts = {}) {
    ...
    const lockedSet = displayOpts.lockedSet instanceof Set
        ? displayOpts.lockedSet
        : new Set(Array.isArray(displayOpts.locked) ? displayOpts.locked : []);
    const lang = typeof currentLang !== "undefined" ? currentLang : "de";
    ...
    const extraChange = displayOpts.onchange
        ? String(displayOpts.onchange).replace(/;?\s*$/, "")
        : "";
    let onchangeAttr = "";
    if (hasMax || extraChange) {
        const parts = [];
        if (hasMax) parts.push(`limitLfCheckboxGrid('${listId}')`);
        if (extraChange) parts.push(extraChange);
        onchangeAttr = ` onchange="${parts.join(";")};"`;
    }

    return `<div class="cc-lf-check-grid" id="${listId}"${dataMax}>
        ${sorted.map(item => {
            const label = typeof item === "string" ? item : item.translationLabel;
            const isLocked = lockedSet.has(label);
            const isChecked = isLocked || selectedSet.has(label);
            const checked = isChecked ? "checked" : "";
            ...
            const textHtml = colorCls
                ...
                : escapeLfHtml(text);
            return `<label${labelCls}><input type="checkbox" value="${label}" ${checked} ${disabled}${lockedAttr}${onchangeAttr}> ${textHtml}</label>`;
        }).join("")}
    </div>`;
}
...
function limitLfCheckboxGrid(listId) {
    ...
    // Fest gesperrte Checkboxen immer angehakt lassen
    boxes.forEach(box => {
        if (box.getAttribute("data-locked") === "1") {
            box.checked = true;
            box.disabled = true;
            const label = box.closest("label");
            if (label) label.classList.add("cc-lf-check-disabled");
        }
    });
    const checkedCount = boxes.filter(b => b.checked).length;
    // Falls Soft-State über dem Max liegt: überschüssige abwählen (nie locked)
    if (checkedCount > max) {
        boxes.filter(b => b.checked && b.getAttribute("data-locked") !== "1")
            .slice(max - boxes.filter(b => b.getAttribute("data-locked") === "1").length)
...
function buildLfFloatOptionsBody(slot) {
    ...
    if (type === "options" && cat === "tools") {
        ...
        if (!surface) {
            return `
                ${buildLfOptionsFilterHeadingHtml("toolsLabel")}
    ...
    if (type === "options" && cat === "asiAndFeat") {
        ...
        return `
            ${buildLfControlRowHtml(
                buildLfOptionsFilterHeadingHtml("talentsLabel"),
                `<select id="ccLfFeatMode" class="dropdown cc-lf-float-input cc-lf-options-filter"
                    ...
                    <option value="all" ${mode === "all" ? "selected" : ""}>${tCC("allLabel")}</option>
                    <option value="featCategory" ${mode === "featCategory" ? "selected" : ""}>${tCC("categoryLabel")}</option>
                    <option value="specific" ${mode === "specific" ? "selected" : ""}>${tCC("selectionLabel")}</option>
                </select>`
            )}
            <div id="ccLfFeatCatWrap" class="cc-lf-control-row" style="display:${mode === "featCategory" ? "flex" : "none"};">
                <label class="cc-lf-float-label" for="ccLfFeatCategoryNumber">${tCC("categoryLabel")}</label>
    ...
    if (type === "simple" && cat === "spellcasting") {
        ...
        const abilityOpts = getLfSpellcastingAbilityOptions().map(attr =>
    ...
    if (type === "simple" && cat === "weaponTraining") {
        ...
        const tab1Cats = new Set(
            normalizeToArray(core.weaponCategoryNumber).map(n => parseInt(n, 10))
        );
        const tab1Props = new Set(
            normalizeToArray(core.weaponPropertyCategoryNumber).map(n => parseInt(n, 10))
        );
            ...
            <div class="cc-lf-param-value-block">
                ${buildLfControlRowHtml(
                    ...
                    `<select id="ccLfSimpleWeaponCatMode" class="dropdown cc-lf-float-input cc-lf-options-filter"
                        ...
                        <option value="all" ${catMode === "all" ? "selected" : ""}>${tCC("allLabel")}</option>
                        <option value="selection" ${catMode === "selection" ? "selected" : ""}>${tCC("selectionLabel")}</option>
                    </select>`,
            ...
            <div class="cc-lf-param-value-block">
                ${buildLfControlRowHtml(
                    ...
                    `<select id="ccLfSimpleWeaponPropMode" class="dropdown cc-lf-float-input cc-lf-options-filter"
                        ...
                        <option value="all" ${propMode === "all" ? "selected" : ""}>${tCC("allLabel")}</option>
                        <option value="selection" ${propMode === "selection" ? "selected" : ""}>${tCC("selectionLabel")}</option>
                    </select>`,
                    "cc-lf-control-row--training-pick"
                )}
    ...
    if (type === "simple" && cat === "armorTraining") {
        const tab1Armor = new Set(
            normalizeToArray(getActiveLfCoreTraitsState().armorCategoryNumber).map(n => parseInt(n, 10))
        );
...
function buildLfSpellcraftGetCantripMaskHtml(slot, cfg) {
    ...
    const dropdowns = picks.map((val, i) => `
        <div class="cc-lf-sc-cantrip-slot${i > 0 ? " cc-lf-sc-cantrip-slot--spaced" : ""}">
            <label class="cc-lf-float-label" for="ccLfScCantrip_${i}">${tCC("cantripLabel")} ${i + 1}</label>
...
function buildLfSpellcraftChooseCantripMaskHtml(slot, cfg) {
    ...
    return `
        ...
        <div class="cc-lf-param-value-block">
            ${buildLfControlRowHtml(
                buildLfOptionsGearHeadingHtml("amountLabel"),
    ...
    return `
        ...
        <div class="cc-lf-param-value-block" style="margin-top:0;">
            ${buildLfSpellcraftFilterBlockHtml({
                ...
                checkboxOnchange: "softRebuildLfSpellcraftPreparedMask();"
            })}
        </div>
        <div class="cc-lf-param-value-block">
            ${buildLfSpellcraftFilterBlockHtml({
...
function readLfSpellcraftListFilterFromDom() {
    return applyLfSpellcraftListFilterFromDom();
}
...
function bindLfSpellcraftMaskHandlers(slot) {
    ...
    const cat = slot.payload.category;
    if (cat === "getCantrip") {
        queryAllActiveLfFloat("#ccLfScSpellLists input").forEach(el => {
    ...
    if (cat === "subclassSpells") {
        queryAllActiveLfFloat("#ccLfScSchools input").forEach(el => {
...
function applyLfSpellcraftListFilterFromDom() {
    const listMode = queryActiveLfFloat("#ccLfScListMode")?.value || "all";
    const spellListLabels = listMode === "selection"
        ? Array.from(queryAllActiveLfFloat("#ccLfScSpellLists input:checked")).map(el => el.value)
        : [];
    return { listMode, spellListLabels };
}

function applyLfSpellcraftSchoolFilterFromDom() {
    const schoolMode = queryActiveLfFloat("#ccLfScSchoolMode")?.value || "all";
    // Bei „Alle“ Checkboxen verwerfen (sonst bleiben Schulen sichtbar/wirksam im State)
    const schoolLabels = schoolMode === "selection"
        ? Array.from(queryAllActiveLfFloat("#ccLfScSchools input:checked")).map(el => el.value)
        : [];
    return { schoolMode, schoolLabels };
}

function applyLfSpellcraftLevelFilterFromDom() {
    const levelMode = queryActiveLfFloat("#ccLfScLevelMode")?.value || "all";
    const levelLabels = levelMode === "selection"
        ? Array.from(queryAllActiveLfFloat("#ccLfScLevels input:checked")).map(el => el.value)
        : [];
...
function applyLfSpellcraftChooseCantripFromDom(slot, { soft = false } = {}) {
    ...
    const pickMin = spec?.pickMin || 1;
    const pickMax = spec?.pickMax || 3;
    let pickCount = parseInt(queryActiveLfFloat("#ccLfScPickCount")?.value, 10) || 0;
    ...
    if (!soft) {
        ...
        if (pickCount < pickMin || pickCount > pickMax) {
            alert(`${tCC("amountLabel")} (${pickMin}–${pickMax})`);
            return false;
        }
    }
...
function readLfAddToSpellbookFromDom(cfg) {
    ...
    const el = queryActiveLfFloat("#ccLfAddToSpellbook");
...
function buildLfSpellcraftSubclassSpellsMaskHtml(slot) {
    ...
    const slots = getLfSlotsForSlot(slot);
    const spellState = (typeof isCustomSubclassEditorActive === "function"
        && isCustomSubclassEditorActive()
        && typeof customSubclassEditorState !== "undefined")
        ? customSubclassEditorState
        : customClassEditorState;
    // Ungültige Wahlen (Tab 4 / Ascending) vor Anzeige bereinigen
    sanitizeLfSubclassSpellsSlotSelections(slot, slots, spellState);
    cfg = slot.payload.optionsConfig || cfg || {};

    const schoolMode = cfg.schoolMode || "all";
    const schoolLabels = cfg.schoolLabels || [];
    const entryLists = getLfSpellcastingEntrySpellListLabels(spellState);
    const allowedGrades = getLfSubclassSpellsAllowedGradeLabels(slot, slots, spellState);
    ...
    const sections = allowedGrades.map(lvl => {
        ...
        // Leere Dropdowns sperren, sobald max. Auswahl erreicht
        const lockEmpty = selectedCount >= maxTotal;
        const dropdowns = picks.map((val, i) => {
            const locked = lockEmpty && !val;
            return `
            <select id="ccLfScSubSp_${lvl}_${i}" class="dropdown cc-lf-float-input cc-lf-options-filter cc-lf-sc-spell-pick"
                ...
                ${locked ? "disabled" : ""}
                onchange="onLfSpellcraftSubclassSpellsPickChange()">
                ${buildLfSpellDropdownOptionsHtml(spells, val, used)}
            </select>
        `;
        }).join("");
...
/**
 * Magieprogression: leere Dropdowns sperren/freigeben je nach max. Zauberanzahl.
 * Belegte Selects bleiben bedienbar (Zurücksetzen auf „Bitte wählen“ entsperrt wieder).
 */
function updateLfSpellcraftSubclassSpellsDropdownLockState() {
    const maxTotal = CUSTOM_CLASS_LF_CONFIG.limits.subclassSpellsPerInvocationMax || 3;
    const selects = Array.from(
        queryAllActiveLfFloat("#ccLfScSubclassSpellsSections select.cc-lf-sc-spell-pick")
    );
    if (!selects.length) return;
    const filledCount = selects.filter(sel => !!sel.value).length;
    const atMax = filledCount >= maxTotal;
    selects.forEach(sel => {
        sel.disabled = atMax && !sel.value;
    });
}
...
function onLfSpellcraftSubclassSpellsPickChange() {
    ...
    const counter = queryActiveLfFloat("#ccLfScSubclassSpellsCounter");
    if (counter) counter.textContent = `${count}/${maxTotal}`;
    // Über Limit: letzte Wahl zurücksetzen
    if (count > maxTotal) {
        const sel = document.activeElement;
        const body = getActiveLfFloatBodyEl();
        if (sel && sel.tagName === "SELECT" && body && body.contains(sel)) {
            sel.value = "";
            applyLfSpellcraftSubclassSpellsFromDom(ctx.slot, { soft: true });
    ...
    updateLfSpellcraftSubclassSpellsDropdownLockState();
}
...
function applyLfSpellcraftSubclassSpellsFromDom(slot, { soft = false } = {}) {
    ...
    // DOM-Grade nur im aktiven Float lesen (keine ID-Kollision mit dem anderen Overlay)
    const gradeNodes = queryAllActiveLfFloat("#ccLfScSubclassSpellsSections .cc-lf-sc-level-block[data-level]");
    ...
    grades.forEach(lvl => {
        ...
        for (let i = 0; i < dropPerGrade; i++) {
            picks.push(queryActiveLfFloat(`#ccLfScSubSp_${lvl}_${i}`)?.value || "");
        }
        selectedByLevel[lvl] = picks;
        picks.filter(Boolean).forEach(lab => allFilled.push(lab));
    });
    ...
    slot.payload.optionsConfig = {
        ...
        spellListLabels: getLfSpellcastingEntrySpellListLabels(spellState)
    };
    slot.payload.optionsConfig = readLfAddToSpellbookFromDom(slot.payload.optionsConfig);
    slot.payload.descriptions = { de: "-", en: "-" };
...
function buildLfAttributeDistributionMaskHtml(slot, cfg) {
    ...
    return `
        ...
        <div class="cc-lf-param-value-block">
            <label class="cc-lf-float-label">${tCC("abilitiesLabel")}</label>
 ...
 */
/** Attribut-Auswahl-Maske für Vordefiniert-Merkmale ohne LEVEL_VAL-Parameter (z. B. Custom Unarmored Defense) */
function buildLfPreDefinedAbilityPickMaskHtml(slot, cfg, pickCount, meta) {
    const n = pickCount || meta?.abilityPickCount || 2;
    const fixed = Array.isArray(meta?.fixedAbilities) ? meta.fixedAbilities.filter(Boolean) : [];
    const freeCount = meta?.freeAbilityPickCount != null
        ? meta.freeAbilityPickCount
        : Math.max(0, n - fixed.length);
    const selected = new Set(normalizeLfPreDefinedSelectedAbilities(cfg.selectedAbilities, meta || { abilityPickCount: n, fixedAbilities: fixed }));
    const attrs = getLfAbilityAttributeLabels();
    const hint = freeCount === 1 && fixed.length
        ? formatLfPickExactHint(freeCount)
        : formatLfPickRangeHint(n, n);
    return `
        <div class="cc-lf-param-value-block" style="margin-top:0;">
            <label class="cc-lf-float-label">${tCC("abilitiesLabel")}</label>
            <p class="cc-lf-float-hint cc-lf-float-hint--after-filter">${hint}</p>
            ${buildLfCheckboxGridHtml("ccLfPreDefAbilityList", attrs, selected, n, {
                locked: fixed
            })}
        </div>
    `;
...
function buildLfPreDefinedParamsMaskHtml(slot, cfg) {
    ...
    const meta = getLfPreDefinedFeatureMeta(label);
    if (meta?.abilityPickCount) {
        return buildLfPreDefinedAbilityPickMaskHtml(slot, cfg, meta.abilityPickCount, meta);
    }
    const params = getLfPreDefinedParameters(label);
    if (!params.length) {
...
function buildLfFreeChoiceRowHtml(index, choice) {
    ...
    const removeBtn = index >= 2
        ? `<button type="button" class="cc-lf-choice-remove" aria-label="${tCC("cfCancelLabel")}"
...
function renumberLfFreeChoiceRows() {
    ...
    Array.from(list.children).forEach((row, i) => {
        ...
        if (showRemove && !removeBtn) {
            ...
            removeBtn.setAttribute("aria-label", tCC("cfCancelLabel"));
...
function openLfFloat(slotId, mode, event) {
    ...
    const overlay = (typeof getActiveLfFloatOverlayEl === "function")
        ? getActiveLfFloatOverlayEl()
        : document.getElementById("ccLfFloatOverlay");
    if (!overlay) return;
    const titleEl = overlay.querySelector(".cc-lf-float-title")
        || document.getElementById("ccLfFloatTitle")
        || document.getElementById("cscLfFloatTitle");
    const hintEl = overlay.querySelector(".cc-lf-float-hint")
        || document.getElementById("ccLfFloatHint")
        || document.getElementById("cscLfFloatHint");
    const bodyEl = overlay.querySelector(".cc-lf-float-body")
        || document.getElementById("ccLfFloatBody")
        || document.getElementById("cscLfFloatBody");
...
function applyLfSimplePreDefinedFromDom(slot, { soft = false } = {}) {
    ...
    if (preDefinedLabel) {
        ...
        if (meta?.requires && !hasLfPreDefinedPrerequisite(slot, meta.requires)) {
            if (!soft) {
        ...
        // Attribut-Auswahl (z. B. Custom Unarmored Defense) nur bei gleichbleibendem Label übernehmen
        const prevSelectedAbilities = slot.payload.optionsConfig?.selectedAbilities;
        const selectedAbilities = (prevLabel === preDefinedLabel && Array.isArray(prevSelectedAbilities))
            ? normalizeLfPreDefinedSelectedAbilities(prevSelectedAbilities, getLfPreDefinedFeatureMeta(preDefinedLabel))
            : normalizeLfPreDefinedSelectedAbilities([], getLfPreDefinedFeatureMeta(preDefinedLabel));
        slot.payload.optionsConfig = { preDefinedLabel, parameterValues, selectedAbilities };
...
function applyLfFloatOptions(slot, { soft = false } = {}) {
    ...
    if (type === "simple" && cat === "spellcasting") {
        ...
        if (!isLfSubclassFeatureSlot(slot)) {
            ...
            && typeof syncCscSpellcastingFromFeatures === "function") {
            syncCscSpellcastingFromFeatures();
        } else {
            // Class-Builder Tab 3: Progression ggf. von UC-Zauberwirken
            syncSpellcastingProgressionFromSlots();
        }
        return true;
    }
    ...
    if (type === "simple" && cat === "armorTraining") {
        ...
        const selectedArmorCategoryNumbers = mode === "selection"
            ...
            : [];
        const pickMin = getLfSimpleCategorySpec("armorTraining")?.pickMin || 1;
        const remainingArmor = getLfRemainingArmorCategoryNumbers(getActiveLfCoreTraitsState());
    ...
    if (type === "simple" && cat === "preDefined") {
        ...
        const preDefMeta = getLfPreDefinedFeatureMeta(preDefinedLabel);
        if (preDefMeta?.abilityPickCount) {
            const n = preDefMeta.abilityPickCount;
            const rawSelected = Array.from(document.querySelectorAll("#ccLfPreDefAbilityList input"))
                .filter(el => el.checked || el.getAttribute("data-locked") === "1")
                .map(el => el.value);
            const selectedAbilities = normalizeLfPreDefinedSelectedAbilities(rawSelected, preDefMeta);
            if (!soft && (selectedAbilities.length < n || new Set(selectedAbilities).size < n)) {
                return lfFailMinOptions();
            }
            slot.payload.optionsConfig = { preDefinedLabel, parameterValues: {}, selectedAbilities };
            const abilityPartial = buildLfPreDefinedClassDataPartial(preDefinedLabel);
            if (abilityPartial) slot.payload.classDataPartial = abilityPartial;
            return true;
        }
...
function renderCustomClassTab2() {
    ...
    const colHeaders = [
        ...
        tCC("categoryLabel", "Kategorie"),
        tCC("ccLfColNameLabel", "Bezeichnung"),
        tCC("ccLfColDescLabel", "Kurz- & Beschreibung"),
        tCC("optionsLabel", "Optionen"),
        tCC("amountLabel", "Anzahl")
    ];
...
function getSubclassDisplayName(sc) {
    ...
    if (name) return name;
    // Standalone Custom Subclass Builder (CSC): nur eine UC, keine sichtbare Nummer nötig
    if (typeof isCustomSubclassEditorActive === "function" && isCustomSubclassEditorActive()) {
        return tCC("subclass");
    }
    return `${tCC("subclass")} ${sc?.subclassCategoryNumber || ""}`.trim();
}
...
function buildScLangBlockHtml(sc, lang, collapsed) {
    ...
    return `
        <div class="custom-class-lang-block" data-lang="${lang}">
            ...
            <div class="${bodyClass}">
                <label for="${nameId}">${tCC("cfNameLabel")} ${lang === activeLang ? '<span class="custom-class-required">*</span>' : ""}</label>
                ...
                <label for="${descId}" style="margin-top:8px;display:block;">${tCC("cfDescLabel")}</label>
...
function buildScFeatureTableHtml(sc, levelsOverride) {
    const slots = sc.levelFeatures || [];
    const levels = Array.isArray(levelsOverride) && levelsOverride.length
        ? levelsOverride.slice()
        : getTab2SubclassLevels(customClassEditorState.levelFeatures);
    if (!levels.length) {
...
function updateCustomClassTab4Ui() {
    ...
    btn.title = unlocked
        ? tCC("cfTabSpellcastingLabel")
        : tCC("cfSpellTabLockedHintLabel");
...
function onCcSpellProgNumberChange(level, field, rawValue) {
    ...
    // Grade können entfallen → Magieprogression-Wahlen bereinigen (nur Class-Builder)
    if (!(typeof isCustomSubclassEditorActive === "function" && isCustomSubclassEditorActive())
        && sanitizeAllLfSubclassSpellsAgainstTab4()
        && document.getElementById("customClassTab3")?.classList.contains("active")) {
        renderCustomClassTab3();
    }
    rerenderActiveSpellProgOwner();
}
...
function openCcSpellListFloat(level, event) {
    ...
    // Standard-Modus Tab4: nur Eingangsstufe; CSC-Preset/Standard: ebenfalls nur Anzeige Start
    const cscActive = typeof isCustomSubclassEditorActive === "function" && isCustomSubclassEditorActive();
    const cscEditable = cscActive
        && customSubclassEditorState?.spellConfigMode === "user";
    if (!cscActive && prog.mode === "standard" && level !== prog.startLevel) return;
    if (cscActive && !cscEditable && level !== prog.startLevel) return;
    ...
    const overlay = (typeof getActiveLfFloatOverlayEl === "function")
        ...
        : document.getElementById("ccLfFloatOverlay");
    if (!overlay) return;
    const title = overlay.querySelector(".cc-lf-float-title");
    const body = overlay.querySelector(".cc-lf-float-body");
    ...
    ccLfFloatContext = {
        slotId: null,
        mode: "ccSpellLists",
        readonly: !!(cscActive && !cscEditable)
    };
    overlay.style.setProperty("display", "flex", "important");
}
...
function commitCcSpellListFloat() {
    ...
    if (!cscActive && level === prog.startLevel) {
    ...
    if (level === prog.startLevel) {
        ...
        enforceSpellProgMonotonicityFrom(level);
        if (!cscActive) sanitizeAllLfSubclassSpellsAgainstTab4();
    }
...
function zeroCcSpellProgValues() {
    ...
    if (!(typeof isCustomSubclassEditorActive === "function" && isCustomSubclassEditorActive())
        && !confirmLfSubclassSpellsResetForTab4Change()) return;
    ensureSpellProgUserRowsInitialized(prog);
...
/**
 * Gemeinsame Zauberprogressions-Tabelle (Tab 4 Class / Tab 2 Subclass).
 * @param {object} prog
 * @param {{ editable?: boolean }} opts
 */
function buildCcSpellProgTableMarkup(prog, opts) {
    ...
    const headCells = [
        ...
        ...Array.from({ length: 9 }, (_, i) => `${i + 1}. ${tCC("spellLevel2Label")}`)
...
function renderCustomClassTab4() {
    ...
    if (!prog.unlocked || !prog.startLevel) {
        container.innerHTML = `<p class="custom-class-hint">${tCC("cfSpellTabLockedHintLabel")}</p>`;
        return;
    }
        ...
        <div class="cc-spell-prog-toolbar">
            ...
            <label class="cc-spell-prog-mode">
                ...
                <select class="dropdown" onchange="onCcSpellProgModeChange(this.value)">
                    ...
                    <option value="user" ${mode === "user" ? "selected" : ""}>${tCC("cfSpellProgModeUserLabel")}</option>
                </select>
                ${editable
...
function buildLfToolSurfaceSelectHtml(slot) {
    ...
    return `<select class="dropdown cc-lf-select" aria-label="${tCC("toolsLabel")}"
